...
For an active-active webhook deployment with multiple StatefulSet replicas and Redis-backed PR ownership, use the separate [Redis replica routing guide](redis-replica-routing.md) and the repository's `examples/redis-ha/atlantis.yaml` manifest. Do not increase the replica count in the single-server manifests below without choosing and configuring a distributed model.
