...
func (e *EmbeddedFileConfig) validateURLs(allowInsecureDev bool) error {
	...
	// Production requires client-certificate authentication on both listeners.
	if !allowInsecureDev {
		if !e.PeerTLS.ClientCertAuth {
			return fmt.Errorf("embedded etcd config: peer_tls.client_cert_auth is required in production")
		}
		if !e.ClientTLS.ClientCertAuth {
			return fmt.Errorf("embedded etcd config: client_tls.client_cert_auth is required in production")
		}
	}
	return nil
}
