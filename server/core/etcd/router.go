...
// NewRouter wires the ownership store, admission store, forwarding client, and
// local executor into the dispatch guard.
func NewRouter(ownership *OwnershipStore, admission *AdmissionStore, client *InternalClient, executor Executor) *Router {
	return &Router{ownership: ownership, admission: admission, client: client, executor: executor}
}
