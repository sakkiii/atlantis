...
// empty/lost PVC").
func ValidateRestartIdentity(manifest *IdentityManifest, fileCfg *EmbeddedFileConfig, dataDir string) error {
	if err := checkManifestConsistency(manifest, fileCfg, manifest.DeploymentID); err != nil {
		return err
	}
	if !manifest.Active {
...
func validateLifecycleIdentity(cfg *Config, fileCfg *EmbeddedFileConfig) error {
	...
	case LifecycleRestart:
		...
		if m.DeploymentID != cfg.DeploymentID {
			return fmt.Errorf("identity manifest: deployment_id %q does not match configured deployment %q", m.DeploymentID, cfg.DeploymentID)
		}
		return ValidateRestartIdentity(m, fileCfg, fileCfg.DataDir)
