...
func TestValidateRestartIdentity_ManifestMismatch(t *testing.T) {
	...
	ErrContains(t, "member_name", ValidateRestartIdentity(m, f, dataDir))

	// Wrong token.
	m = validIdentityManifest()
	m.InitialClusterToken = "other"
	ErrContains(t, "initial_cluster_token", ValidateRestartIdentity(m, f, dataDir))

	// Inactive manifest.
	m = validIdentityManifest()
	m.Active = false
	ErrContains(t, "requires an active identity manifest", ValidateRestartIdentity(m, f, dataDir))

	// Missing persisted ids in manifest.
	m = validIdentityManifest()
	m.ClusterID = ""
	ErrContains(t, "cluster_id and member_id", ValidateRestartIdentity(m, f, dataDir))

	// Consistent manifest but the member dir is absent -> refuse (empty/lost PVC).
	m = validIdentityManifest()
	ErrContains(t, "member directory", ValidateRestartIdentity(m, f, dataDir))
}
...
func TestValidateRestartIdentity_PersistedIdentity(t *testing.T) {
	...
	m := validIdentityManifest()
	Ok(t, ValidateRestartIdentity(m, f, dataDir))
	...
	ErrContains(t, "member_id mismatch", ValidateRestartIdentity(m, f, dataDir))

	// Wrong persisted cluster id -> refuse.
	m = validIdentityManifest()
	m.ClusterID = types.ID(0x111).String()
	ErrContains(t, "cluster_id mismatch", ValidateRestartIdentity(m, f, dataDir))
}
