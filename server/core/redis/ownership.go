...
func (s *OwnerStore) renewOwned(ctx context.Context) {
	...
	for redisKey, record := range owned {
		renewed, err := s.renewSerialized(ctx, redisKey, record.serialized)
		if err != nil {
	...
	if renewalErr != nil {
		s.markRenewalFailure(renewalErr)
...
func (s *OwnerStore) markRenewalFailure(err error) {
	s.healthMu.Lock()
	if s.renewalFailureSince.IsZero() {
		s.renewalFailureSince = time.Now()
	}
