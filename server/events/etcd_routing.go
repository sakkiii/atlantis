...
func (r *EtcdCommandRouter) RunAutoplanCommand(baseRepo models.Repo, headRepo models.Repo, pull models.PullRequest, user models.User) {
	...
	r.dispatch("autoplan", baseRepo, pull.Num, payload)
}
...
func (r *EtcdCommandRouter) RunCommentCommand(baseRepo models.Repo, maybeHeadRepo *models.Repo, maybePull *models.PullRequest, user models.User, pullNum int, cmd *CommentCommand) {
	...
	r.dispatch("webhook", baseRepo, pullNum, payload)
}

// dispatch builds the command envelope and routes it. On any non-accepted result
// it fails closed with a user-visible comment rather than silently dropping the
// command or letting an arbitrary replica run it.
func (r *EtcdCommandRouter) dispatch(sourceKind string, baseRepo models.Repo, pullNum int, payload routedPayload) {
	...
	cmd := etcd.Command{
		Identity: etcd.CommandIdentity{
			...
			// Without a threaded provider delivery ID, each ingress is a distinct
			// admission. This preserves the single-owner execution guarantee; it
			// does not deduplicate duplicate provider deliveries (a follow-up).
